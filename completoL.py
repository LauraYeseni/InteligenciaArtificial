...
# Seleccionar las características "Age" y "Survived" para el conjunto de datos heart
X = df[["age", "output"]].values

# plot data
plt.scatter(X[X[:, 1] == 0, 0], X[X[:, 1] == 0, 1], color='red', marker='o', label='No enfermo')
plt.scatter(X[X[:, 1] == 1, 0], X[X[:, 1] == 1, 1], color='blue', marker='x', label='Enfermo')
plt.xlabel('Age')
plt.ylabel('Survived')
